function dijkstraConRetardos(red, origen, destino) {
    ...
    var totalDelay = parseInt(origen.delay);
    ...
    while (anterior[nodoActual] !== null) {
        ...
        totalDelay += parseInt(red.find(n => n.id === nodoActual).delay); // Sumar el delay del nodo actual al tiempo total
    ...
    return { ruta: rutaMinima.map(id => red.find(nodo => nodo.id === id)), totalDelay};
}